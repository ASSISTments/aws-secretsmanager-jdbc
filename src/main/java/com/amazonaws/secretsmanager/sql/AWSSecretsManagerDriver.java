...
/**
 * <p>
 * Provides support for accessing SQL databases using credentials stored within
 * AWS Secrets Manager. If this functionality is desired, then a subclass of
 * this class should be specified as the JDBC driver for an application.
 * </p>
 *
 * <p>
 * The driver to propagate <code>connect</code> requests to should also be
 * specified in the configuration. Doing this will cause the real driver to be
 * registered once an instance of this driver is made (which will be when this
 * driver is registered).
 * </p>
 *
 * <p>
 * This base class registers itself with the <code>java.sql.DriverManager</code>
 * when its constructor is called. That means a subclass only needs to make a
 * new instance of itself in its static block to register.
 * </p>
 *
 * <p>
 * This does not support including the user (secret ID) and password in the jdbc
 * url, as JDBC url formats are database specific. If this functionality is
 * desired, it must be implemented in a subclass.
 * </p>
 *
 * <p>
 * Ignores the password field, drawing a secret ID from the user field. The
 * secret referred to by this field is expected to be in the standard JSON
 * format used by the rotation lambdas provided by Secrets Manager:
 * </p>
 *
 * <pre>
 * {@code
 ...
 *
 * <p>
 * Here is a list of the configuration properties. The subprefix is an
 * implementation specific String used to keep the properties for different
 * drivers separate. For example, the MySQL driver wrapper might use mysql as
 * its subprefix, making the full property name for the realDriverClass for the
 * MySQL driver wrapper drivers.mysql.realDriverClass (all Driver properties
 * will be prefixed with "drivers."). This String is defined by the method
 * <code>getPropertySubprefix</code>.
 * </p>
 *
 * <ul>
 * <li>drivers.<i>subprefix</i>.realDriverClass - (optional) The class name of
 * the driver to propagate calls to. If not specified, default for
 * <i>subprefix</i> is used</li>
 * </ul>
 */
public abstract class AWSSecretsManagerDriver implements Driver
{
   ...
   * system properties as defaults. Instantiates the secret cache with default
   * options.
   */
  ...
  /**
   * Constructs the driver setting the properties from the properties file using
   * system properties as defaults. Sets the secret cache to the cache that was
   * passed in.
   *
  ...
  protected AWSSecretsManagerDriver(SecretCache cache)
  {
    ...
    String vpcEndpointUrl =
      config.getStringPropertyWithDefault(PROPERTY_PREFIX + "." + PROPERTY_VPC_ENDPOINT_URL, null);
    String vpcEndpointRegion =
      config.getStringPropertyWithDefault(PROPERTY_PREFIX + "." + PROPERTY_VPC_ENDPOINT_REGION, null);
  ...
  /**
   * Constructs the driver setting the properties from the properties file using
   * system properties as defaults. Instantiates the secret cache with the passed
   * in client builder.
   *
  ...
  /**
   * Constructs the driver setting the properties from the properties file using
   * system properties as defaults. Instantiates the secret cache with the
   * provided AWS Secrets Manager client.
   *
  ...
  /**
   * Gets the "subprefix" used for configuration properties for this driver. For
   * example, if this method returns the String, "mysql", then the real driver
   * that this will forward requests to would be set to
   * drivers.mysql.realDriverClass in the properties file or in the system
   * properties.
   *
  ...
  /**
   * Replaces <code>SCHEME</code> in a jdbc url with "jdbc" in order to pass the
   * url to the real driver.
   *
   * @param  jdbcUrl                  The jdbc url with <code>SCHEME</code> as the scheme.
   *
   * @return                          String The jdbc url with the scheme changed.
   *
   * @throws IllegalArgumentException When the url does not start with
   *                                    <code>SCHEME</code>.
   */
  ...
  protected void ensureRealDriverRegistered()
  {
    ...
    try
    {
      cls = (Class<Driver>) getClass().getClassLoader().loadClass(this.realDriverClass);
  ...
  /**
   * Returns an instance of the real <code>java.sql.Driver</code> that this should
   * propagate calls to. The real driver is specified by the realDriverClass
   * property.
   *
   * @return                       Driver The real <code>Driver</code> that calls should be propagated
   *                               to.
   *
  ...
  {
    this.ensureRealDriverRegistered();
    ...
    throw new IllegalStateException(
      "No Driver has been registered with name, " + this.realDriverClass + ". Please check your system properties or "
  ...
  public boolean acceptsURL(String url) throws SQLException
  {
    ...
    {
      // If this is a URL in our SCHEME, call the acceptsURL method of the wrapped
      // driver
    ...
    {
      // We accept a secret ID as the URL so if the config is set, and it's not a JDBC
      // URL, return true
      return true;
    }
  }
  ...
  /**
   * Determines whether or not an <code>Exception</code> is due to an
   * authentication failure with the remote database. This method is called during
   * <code>connect</code> to decide if authentication needs to be attempted again
   * with refreshed credentials. A good way to implement this is to look up the
   * error codes that <code>java.sqlSQLException</code>s will have when an
   * authentication failure occurs. These are database specific.
   *
   * @param  exception The <code>Exception</code> to test.
   *
   * @return           boolean Whether or not the <code>Exception</code> indicates that the
   *                   credentials used for authentication are stale.
   */
  public abstract boolean isExceptionDueToAuthenticationError(Exception exception);

  /**
   * Construct a database URL from the endpoint, port and database name. This
   * method is called when the <code>connect</code> method is called with a secret
   * ID instead of a URL.
   *
  ...
  /**
   * Calls the real driver's <code>connect</code> method using credentials from a
   ...
   *                                The user and password fields will be replaced with
   *                                the credentials retrieved from Secrets Manager.
   * @param  credentialsSecretId  The friendly name or ARN of the secret that stores
   *                                the login credentials.
   *
   * @return                      Connection A database connection.
   *
   * @throws SQLException         If there is an error from the driver or
   *                                underlying database.
   * @throws InterruptedException If there was an interruption during secret
   *                                refresh.
   */
    ...
    throws SQLException, InterruptedException
  {
    ...
    while (retryCount++ <= MAX_RETRY)
    {
      ...
      {
        // Most likely to occur in the event that the data is not JSON. This is more of
        // a user error.
  ...
  public Connection connect(String url, Properties info) throws SQLException
  {
    ...
    String unwrappedUrl = "";
    if (url.startsWith(SCHEME))
    { // If this is a URL in the correct scheme, unwrap it
    ...
    { // Else, assume this is a secret ID and try to retrieve it
      ...
      if (StringUtils.isNullOrEmpty(secretString))
      {
        throw new IllegalArgumentException(
          "URL " + url + " is not a valid URL starting with scheme " + SCHEME + " or a valid retrievable secret ID ");
      }
